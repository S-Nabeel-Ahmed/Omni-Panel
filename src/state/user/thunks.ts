...
export const logoutWallet = (address: string) => async (dispatch: AppDispatch, getState: () => RootState) => {
  const { accessToken, data: user } = getState().user
  if (address) storage.removeItem(`MM_TokenInfo_${address.toLowerCase()}`)
  if (user?.publicAddress) storage.removeItem(`MM_TokenInfo_${user.publicAddress.toLowerCase()}`)
  else if (accessToken) {
    try {
      const _tmpUser = await getLoggedInUser(accessToken)
      storage.removeItem(`MM_TokenInfo_${_tmpUser?.publicAddress?.toLowerCase()}`)
    } catch (error: any) {
      console.error(error)
